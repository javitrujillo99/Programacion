...
/* PETICION PARA ELEGIR JUGADORES */
do {
  var jugadores = prompt("Escriba UNO para modo 1 jugador, DOS para 2 jugadores");
  if (jugadores == "UNO") {
    dosJugadores = false;
  }
  else if (jugadores == "DOS") {
    dosJugadores = true;
  }
  else {
    alert("DEBE ESCRIBIR UNO O DOS CON MAYUSCULAS.");
  }
} while (jugadores != "UNO" && jugadores != "DOS");
...
function gato() {
  ...
  /* Creacion de las casillas para las fichas */
...
function verificaFin(O, X) {
  ...
  if (O == 3) {
    ...
    if (dosJugadores == false) {
      vidas--;
      mensaje("¡Perdiste! Le quedan " + vidas + " vidas.");
    } else {
  ...
  if (!fin) {
    if (tiradas < 9) {
      ...
      if (dosJugadores == false) {
        fin = true;
        vidas--;
        mensaje("¡Empate! Le quedan " + vidas + " vidas.");
      } else {
        fin = true;
        mensaje("¡Empate!");
      }
  ...
  if (vidas == 0) {
    mensaje("NO TIENES VIDAS. FIN DEL JUEGO.");
  }
  return fin;
}